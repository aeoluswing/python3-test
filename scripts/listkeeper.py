...
def choose_file():
    ...
    if not enter_filename:
        ...
        index = get_integer("Specify file's number (or 0 to create "
                            "a new one)", "index", maximum=len(files),
                            allow_zero=True)
        if index == 0:
            enter_filename = True
        else:
...
def load_list(filename):
    ...
    try:
        ...
        print("ERROR: failed to load {filename}: {err}".format(**locals()))
        return []
    else:
        print("Success load {filename}".format(**locals()))
    finally:
        if fh is not None:
            fh.close()
    return items
...
def delete_item(items,dirty):
    index = get_integer("Delete item number (or 0 to cancel)",
                       "index",minimum_length=0,
                       maximum_length=len(items),
                       allow_zero=True)
    if index != 0:
        del items[index - 1]
        dirty = True
...
def save_list(filename,items,terminateing = False):
    fh = None
    try:
        ...
        print ("ERROR: failed to save {filename}: {err}".format(**locals()))
        return True
    else:
